USE [VISTA]
GO
/****** Object:  StoredProcedure [dbo].[sp_Reporte_Generar]    Script Date: 14/07/2026 02:12:04 p. m. ******/
SET ANSI_NULLS ON
GO
...
--      nuevo de SO130120 que pidió Fernando).
...
BEGIN
    ...
    -- de fechas). Un requisito se da por cumplido si el filtro trae
    -- Valor (filtros tipo Igual/Contiene, ej. RFC) O si trae
    -- ValorDesde/ValorHasta (filtros tipo Rango, ej. fechas — estos
    -- nunca llenan Valor, así que si solo se checara Valor un filtro
    -- de rango obligatorio jamás se daría por satisfecho).
    --
    -- El mensaje de error incluye el/los CampoCatalogoId + Alias que
    -- realmente espera (sacados de Rep_CatGrupoRequisitos), en vez de
    -- un texto genérico — si el Id que manda el API no es el mismo
    -- que aquí se pide, se ve al toque sin tener que adivinar.
    ------------------------------------------------------------------
